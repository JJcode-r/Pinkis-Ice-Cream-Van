...
import { X, Mail, Phone, CalendarCheck, MapPin } from 'lucide-react';
...
const InquiryModal = ({ isOpen, onClose }: InquiryModalProps) => {
    ...
    // --- SUCCESS CONTENT (Extracted from previous turn) ---
    ...
    // --- FORM CONTENT (Extracted from previous turn) ---
    const FormContent = (
        <motion.div
            key="form"
    ...
    return (
        ...
        >
            ...
            <motion.div 
                ...
                className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto relative" // Changed max-w-2xl to max-w-lg for better mobile fit
...
// Simple Input Helper Component (kept for completeness, though not used by the new form's structure)
const Input = ({ label, type, name, value, onChange, required, icon }: InputProps) => (
    <div>
        ...
        <div className="relative flex items-center">
            ...
            <input
                ...
                onChange={onChange}
                required={required}
                className={`w-full px-4 py-3 border border-pink-200 rounded-xl focus:ring-pink-500 focus:border-pink-500 transition shadow-sm ${icon ? 'pl-10' : ''}`}
            />
        </div>
    </div>
);
...
// --- Main App Component (formerly FAQSection) ---
const App = () => {
